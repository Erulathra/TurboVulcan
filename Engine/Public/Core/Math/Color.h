...
namespace Turbo
{
	namespace ELinearColor
	{
		...
		constexpr u32 LinearColorToHex(glm::float4 color)
		{
			color = glm::saturate(color);
			return
				static_cast<u32>(color.r * 0xFF) << 24
				| static_cast<u32>(color.g * 0xFF) << 16
				| static_cast<u32>(color.b * 0xFF) << 8
				| static_cast<u32>(color.a * 0xFF) << 0;
		}

		constexpr glm::float4 HexToLinearColor(u32 hex)
		{
			return glm::float4 {
				(hex >> 24) & 0xFF,
				(hex >> 16) & 0xFF,
